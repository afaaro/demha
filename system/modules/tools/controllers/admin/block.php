...
class ToolsAdminBlock extends Controller
{
    ...
    public function indexAction(): void
    {
        ...
        $dbBlocks = $this->db->query('SELECT * FROM #__block ORDER BY `region` ASC, `weight` ASC, `id` ASC')->rows;

        // 2. Get STATIC blocks from the Block class
        $staticBlocks = $this->getStaticBlocksList();

        // 3. Merge: Static blocks come first, then DB blocks; both sorted by region→weight
        $blocks = array_merge($staticBlocks, $dbBlocks);
        usort($blocks, fn($a,$b) =>
            ($a['region'] ?? '') <=> ($b['region'] ?? '')
        ...
        echo $this->view->inline(static function ($view) use ($blocks): void {
            ...
            if (empty($blocks)) {
                ...
                foreach ($blocks as $block) {
                    $isStatic = ($block['is_static'] ?? false);
                    $id = $isStatic ? $block['id'] : (int) $block['id'];
                    $status = (int) ($block['status'] ?? 0) === 1;
                    $typeLabel = $isStatic
                        ? '<span class="badge bg-info text-dark">Static</span> ' . $view->e((string) ($block['body_type'] ?? ''))
                        : $view->e((string) ($block['body_type'] ?? ''));

                    echo '<tr class="' . ($isStatic ? 'table-light' : '') . '">';
                    echo '<td><strong>' . $view->e((string) ($block['title'] ?? '')) . '</strong>' . ($isStatic ? '<br><small class="text-muted">Code-defined</small>' : '') . '</td>';
                    ...
                    if ($isStatic) {
                        echo '<span class="text-muted small px-2">—</span>';
                    } else {
    ...
    public function editAction(): void
    {
        ...
        if (empty($block)) {
            Notify::error('Block not found. Static blocks cannot be edited — modify the code where they are registered.');
    ...
    private function saveForm(?array $block): void
    {
        ...
        if ($this->request->isPost()) {
            ...
            if (empty($errors)) {
                $data = [
                    ...
                    'status'     => $this->request->post('status', 'raw', 0) ? 1 : 0,
        ...
        echo $this->view->inline(static function ($view) use ($form, $errors, $regions, $bodyTypes, $visibilityModes, $statusChecked, $formAction, $heading): void {
            ...
            echo '<div class="d-flex gap-2 mt-4">'
                ...
                . ' <a class="btn btn-outline-secondary" href="' . route_url('tools/admin/block') . '">Cancel</a></div>';
            echo $form->end();
        }, 'admin');
    }

    /**
     * Pull registered static blocks from the Block class
     */
    private function getStaticBlocksList(): array
    {
        // Use reflection to read the private static $staticBlocks property
        $reflection = new \ReflectionClass($this->block);
        $prop = $reflection->getProperty('staticBlocks');
        $prop->setAccessible(true);
        $staticBlocks = $prop->getValue();

        $normalized = [];
        foreach ($staticBlocks as $id => $b) {
            $normalized[] = [
                'id'         => $id,
                'title'      => $b['title'] ?? '(no title)',
                'region'     => $b['region'] ?? '',
                'body_type'  => $b['body_type'] ?? 'static',
                'visibility' => $b['visibility'] ?? 'include',
                'weight'     => $b['weight'] ?? 0,
                'status'     => $b['status'] ?? 1,
                'is_static'  => true,
            ];
