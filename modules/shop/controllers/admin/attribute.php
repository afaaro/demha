...
class ShopAdminAttribute extends Controller {
    public function indexAction(): void
    {
        $groups = $this->db->query("
            SELECT g.*, (SELECT COUNT(id) FROM #__shop_option_value WHERE group_id = g.id) AS total_values FROM #__shop_option_group g ORDER BY g.name ASC
        ")->rows;
        ...
        echo $this->view->inline(function ($view) use ($groups) {
            ...
            if (empty($groups)) {
                ...
                foreach ($groups as $g) {
                    ...
                    echo "<td>" . (int)$g['total_values'] . " options</td>";
                    echo "<td>";
    ...
    public function addAction(): void
    {
        ...
        if ($this->form->isSubmitted()) {
            if ($this->form->isValid()) {
                ...
                // Insert new group
                $this->db->insert('shop_option_group', [
                    'name' => $data['name'],
                    'type' => $data['type'],
                ...
                // Delete old values (if any) and insert new ones
    ...
    public function deleteAction(): void
    {
        ...
        $group = $this->db->findOne('#__shop_option_group', $id);
        if (!$group || ((int)$group['store_id'] !== $this->store_id && $group['store_id'] !== null)) {
            Notify::error('Attribute group not found.');
            redirect($this->url->to('shop/admin/attribute'));
            return;
        }

        // Cascade delete (values will be removed due to FK)
        $this->db->delete('#__shop_option_group', ['id' => $id]);
    ...
    private function renderForm(array $group = [], array $values = []): void
    {
        ...
        echo $this->view->inline(function ($view) use ($isEdit, $group, $typeOptions, $valueHtml) {
            ...
            echo $view->form->input('name', [
                ...
                'rules' => 'required|(!$isEdit || unique:shop_option_group,name,' . ($isEdit ? $group['id'] : 'NULL') . ')',
